...
static void fnc_tx(const unsigned char *data, unsigned long len) {
	int sent = zmq_send(publisher, data, len, 0);
	if (sent < 0) {
		zmq_close(publisher);
...
static PyObject * pycsp_basic_rx(PyObject * self, PyObject * args) {
    Py_buffer buff;
...
static PyObject * pycsp_basic_init(PyObject * self, PyObject * args) {
    const char *if_name = CSP_IF_BASIC_DEFAULT_NAME;
    ...
    int res = csp_basic_add_interface(if_name, *fnc_tx, &iface);
    if (res != CSP_ERR_NONE) {
        return PyErr_Error("csp_basic_add_interface()", res);
    }
...
static PyMethodDef methods[] = {
    ...
    /* BASIC interface */
    {"basic_init", pycsp_basic_init, METH_VARARGS, ""},
    {"basic_rx", pycsp_basic_rx, METH_VARARGS, ""},
